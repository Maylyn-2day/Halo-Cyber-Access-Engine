...
namespace {
    ...
    bool splitAndValidateLine(const char* line, uint32_t length, ParsedLine& parsed) {
        FieldView fields[7];
        ...
        for (uint32_t i = 0; i <= length; ++i) {
            if (i == length || line[i] == ',') {
                ...
                fields[fieldIndex] = FieldView(fieldStart, static_cast<uint32_t>(&line[i] - fieldStart));
                ++fieldIndex;
                fieldStart = &line[i + 1];
            }
        }
    ...
    bool looksLikeHeader(const char* line, uint32_t length) {
        const char expected[] = "user_id,device_id,app_id,resource_id,event_type,location,timestamp";
